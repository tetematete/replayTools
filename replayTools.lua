    ...
    ui.tabItem("Replay Data", function ()
      ui.tabBar("ReplayControls", function()
        ui.tabItem("Events",function ()
          ...
          local replayStartTime = (resultsFile["Laps"][1]["Timestamp"] - (resultsFile["Laps"][1]["LapTime"]/1000))-resultsFile["SessionConfig"]["wait_time"]
          ...
          if value["Driver"]["Name"] ~= resultsFile["Events"][math.min(index+1,#resultsFile["Events"])]["OtherDriver"]["Name"] then
            if ui.button(ac.lapTimeToString(((value["Timestamp"]-replayStartTime-1)-((value["Timestamp"]-replayStartTime-1)*0.0013))*1000).." " ..value["Driver"]["Name"] .. " " .. value["Type"] .. " " .. value["OtherDriver"]["Name"]) then
              --ac.log(((value["Timestamp"]-replayStartTime-1)-((value["Timestamp"]-replayStartTime-1)*0.004)*1000)/sim.replayFrameMs)
              ac.setReplayPosition((((value["Timestamp"]-replayStartTime-2)-((value["Timestamp"]-replayStartTime-2)*0.0013))*1000)/sim.replayFrameMs,1)
...
if input == nil then
  ...
  web.get(settings.URL .. "/api/results/list.json?q=" .. query .. "&sort=" .. sort .. "&page=" .. pageNumber, function(err, response)
    ac.debug("error", error)
    resultsPage = JSON.parse(response.body)
  end)
  ac.log(settings.URL .. "/api/results/list.json?q=" .. query .. "&sort=" .. sort .. "&page=" .. page)
end
